...
urgrep 在 tramp project 上搜索显示颜色有问题，但是 rg 可以，如果解决不了就弃用 urgrep。
https://github.com/jimporter/urgrep/issues/12
...
gofmt and goimprots don’t support format region by format-all, so apheleia is best for now.
...
使用 =visual-fill-column-mode= 进行软折行，避免一行太长不方便游览。
