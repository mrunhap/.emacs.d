...
*** TODO help shortdoc
Why this works under macos but not work under linux(can’t find function)?
#+begin_src emacs-lisp
...
(defun eat/timestamp (begin end)
  ...
  (interactive "r") ; 获取当前选择的区域作为(begin, end)参数
...
#+end_example
Check =rime-share-data-dir(eg //usr/share/rime-data)= and =rime-user-data-dir (eg ~/.config/emacs/rime)=.
https://github.com/DogLooksGood/emacs-rime/issues/79#issuecomment-609560097
...
(setq company-idle-delay 0.2) ;; time for snippet expand
...
(setq company-frontends '(company-pseudo-tooltip-frontend  ; always show candidates in overlay tooltip
...
(with-eval-after-load "company"
  ...
  (define-key company-mode-map [remap completion-at-point] 'company-complete-selection)
...
;; NOTE company-box will have tab-bar in childframe, so use posframe
(eat/pkg company-posframe "https://github.com/tumashu/company-posframe")
(when (display-graphic-p)
...
** tabnine
#+begin_src emacs-lisp
(eat/pkg company-tabnine)
(autoload #'company-tabnine-install-binary "company-tabnine" nil t)
(when (file-directory-p "~/.TabNine/")
...
#+end_src
*** TODO Flymake
Cause redisplay error under linux with go project.
...
#+end_src
** TODO REPL programming for all languages
If the cli program not found, it cause major mode can’t load.
#+begin_src emacs-lisp
(eat/pkg repl-driven-development)


;; (with-eval-after-load 'go-mode
;;   (repl-driven-development [C-x C-g] "yaegi"))
#+end_src
* Tools
** Visual Fill Column
