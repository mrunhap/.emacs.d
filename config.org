...
(defvar eat/all-packages (if (daemonp) t nil))
...
(defmacro eat/pkg (package &optional vc)
  ...
  `(when eat/all-packages
     (message "require: %s" ',package)
     (require ',package)))
#+end_src
*** auto save
#+begin_src emacs-lisp
...
(with-eval-after-load 'go-ts-mode
  ...
  ;; FIXME
  ;; Why treesit-check-indent still use 8 as indent offset as I alreay
  ;; set go-ts-mode-indent-offset to 4? And electric-indent-mode make
  ;; enter after) } indent 8 space too.
  ;; workaround
  (bind go-ts-mode-map
