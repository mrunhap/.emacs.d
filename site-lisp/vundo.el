...
;; q    to quit, you can also use C-g
;;
...
;; makes some change and come back, the vundo buffer is not updated
;; automatically. When you invoke any vundo command, instead of
;; performing that command, vundo updates the buffer to catch up with
;; the original buffer. After that, if you still want to perform that
;; command, invoke it again.
  ...
  (cl-loop
   ...
   do (let ((prev-undo (undo--last-change-was-undo-p
        ...
        (pcase prev-undo
          ...
          ;; This modification either is a region-undo, or not an
          ;; undo. We treat both the same.
          ((or 'undo-in-region _)
